plotBars2 <- function(inpr, mycl.z, mytitle, ylab="Precipitation (mm/hour)"){
    require(ggplot2)
    require(raster)
    ...
    myresult[myresult$Landcover == "orography" & myresult$zone == 4, c(3:6)] <- NA
    myresult$zone <- as.factor(myresult$zone)
    myLUT <- getLUT(nBndClass=1)
    p1 <- ggplot(myresult, aes(x=zone, y=mean, fill=Landcover)) + 
            geom_bar(position=position_dodge(), stat="identity") +
            scale_fill_manual(values=as.character(myLUT$Colours)[c(3,2,4,1,5)]) +
            labs(title=mytitle, y=ylab, x="Zone") +

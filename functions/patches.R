...
mypatch <- function(threshold=100, inbr=inbr, id="s", land_simple, timestep="avg", indatadir="/data/local/hadhy/ModelData/WAFR/", dlresultsdir="/data/local/hadhy/Projects/InternalSabbatical/Results/", spp=spp, sppa=sppa, overwrite=T){
	...
	for (f in 1:nlayers(inbr)){
		mydate <- getZ(inbr)[f]
        if (!inherits(mydate, "POSIXct")){ mydate <- as.POSIXct(mydate)}
		print(mydate)
		...
		if (!file.exists(mcs.f) | overwrite==T){
		    ...
		    if (timestep=="5min"){
		        ...
		        r <- calc(inbr[[f]], fun=function(x){x*60*5}, filename=paste(scratchdir,"myrain.tif",sep=""), format="GTiff", overwrite=T)
		        #                 browser()
		        # Do the clump. 10min precip > 0.16667 ~ 1mm per hour (i.e. 10 * 1/60)
		        myclump <- clump(r > 0.08333333, directions=4, gaps=F)
		    }
...
#                 browser()
	...
	# 			r <- inbr[[f]]
			...
			# Set plot bounds
			xbnd <- c(extent(spp)@xmin,extent(spp)@xmax)
...
# 			plot(sppa, add=T)
            ...
            rat$code <- c("Pop","MCS")
            levels(mcs) <- rat
            levelplot(mcs, margin=F, col.regions=c("green","orange"), main=format(mydate, "%b%d %H:%M")) + latticeExtra::layer(sp.polygons(land_simple))

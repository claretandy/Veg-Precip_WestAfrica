...
vegPrep <- function(model.nm, id="s", myproj="ll", myveg, myorog, mylandfrac, land_simple, sppa, spp.r, plots=F, vegThreshold=0.3, bndDef=2, nBuf=3, nBndClass=1, overwrite=T){
    ...
    # nBuf = Dimensions of the matrix for identifying buffers. E.g. 3x3 matrix means that if 1 boundary pixel is within a 3x3 window, the centre of that matrix is classified as boundary. Values: either 3, 5, or 7. Default=3
...
# 	spp.r <- raster::mask(spp.r, orogmsk)
    ...
    myclfile <- paste(outdir,"mycl_masked_",myproj,"_",vt,"vegThr_",bndDef,"bndDef_",nBuf,"nBuf.tif",sep="")
	if (!file.exists(myclfile) | overwrite==T){
        ...
        focalin <- mycl
        focalin[focalin == 3] <- 2
		mybnds <- focal(focalin, w=matrix(1, nrow=3, ncol=3), fun=function(x){
            z <- table(x); 
            ifelse(z[1]>bndDef & z[2]>bndDef,3,0)
            }, filename=paste(outdir,"mycl_focal_",myproj,"_",vt,"vegThr_",bndDef,"bndDef.tif",sep=""), format="GTiff", overwrite=T)
		mybnds.buf <- focal(mybnds, w=matrix(1, nrow=nBuf, ncol=nBuf), fun=max, na.rm=T, filename=paste(outdir,"mybnds_focal_",myproj,"_",vt,"vegThr_",bndDef,"bndDef.tif",sep=""), format="GTiff", overwrite=T)
		mycl[mybnds == 3] <- 4

...
tracker3 <- function(threshold=1000, inmcs, inprecip, id="s", timestep="10min", indatadir="/data/local/hadhy/ModelData/WAFR/", dlresultsdir="/data/local/hadhy/Projects/InternalSabbatical/Results/", overwrite=T){
    ...
    resultsfile <- paste(dlresultsdir,"Tracking/djzx",id,"_",threshold, "km_",timestep,".RData", sep="")
    ...
    #         myxtab.p1 <- mycrosstab(rt.clump, rtp1.clump)
            ...
            trk <- regular_tracking3(rtm1.clump, rt.clump, tnew, myxtab, results, uniqID, mydate)
    #         print(unique(trk[[1]]))
            trk <- dissipation3(rtm1.clump, trk[[2]], trk[[1]], myxtab, trk[[3]], trk[[4]], mydate, mydate.tm1) 
    #         print(unique(trk[[1]]))
            trk <- generation3(rtm1.clump, trk[[2]], trk[[1]], myxtab, trk[[3]], trk[[4]], mydate) 
    #         print(unique(trk[[1]]))
            trk <- merging3(rtm1.clump, trk[[2]], trk[[1]], myxtab, trk[[3]], trk[[4]], mydate, mydate.tm1) 
    #         print(unique(trk[[1]]))
            trk <- splitting3(rtm1.clump, trk[[2]], trk[[1]], myxtab, trk[[3]], trk[[4]], mydate)
            trk <- remaining3(rtm1.clump, trk[[2]], trk[[1]], myxtab, trk[[3]], trk[[4]], mydate)
...
regular_tracking3 <- function(tm1, t, tnew, myxtab, results, uniqID, mydate){
...
#     no.ovrcl.tm1.t <- apply(myxtab.nz, 2, FUN=function(x){z <- which(x>0); return(length(z))})
    ...
    # keep.t are all the clusters in t that overlap with only 1 cluster in tm1
#     print( paste("Rownames:",paste(rownames(myxtab), collapse=" ")) )
#     print( paste("Colnames:",paste(colnames(myxtab), collapse=" ")) )
    
    myxtab.nz <- myxtab[which(rownames(myxtab)>0),which(colnames(myxtab)>0)]
    
    if (inherits(myxtab.nz, "integer")){ 
        try( keep.t <- as.numeric(rownames(myxtab[which(myxtab.nz > 0)+1,])) )
        try( keep.tm1 <- as.numeric(colnames(myxtab)[colnames(myxtab)>0]) )
    }
    if (inherits(myxtab.nz, "data.frame")){ 
        try( keep.t <- apply(myxtab[which(rownames(myxtab)>0),which(colnames(myxtab)>0)], 1, FUN=function(x){x <- x[x>0]; return(length(x))}) )
        try( keep.t <- as.numeric(attributes(keep.t[keep.t == 1])$names) )
        
        # keep.tm1 are all the clusters in tm1 that overlap with only 1 cluster in t
        try( keep.tm1 <- apply(myxtab[which(rownames(myxtab) %in% keep.t),which(colnames(myxtab)>0)], 2, FUN=function(x){x<-x[as.numeric(attributes(x)$names)>0]; x <- x[x>0]; return(length(x))}) )
        try( keep.tm1 <- as.numeric(attributes(keep.tm1[keep.tm1 == 1])$names) )
        
    }
    
    if (!inherits(keep.t, "try-error")){ 
        
#         print( paste("keep.tm1:",paste(keep.tm1, collapse=" ")) )
        myxtab.regtrk <- myxtab[which(as.numeric(dimnames(myxtab)[[1]]) %in% keep.t), which(as.numeric(dimnames(myxtab)[[2]]) %in% keep.tm1)]
        print("myxtab.regtrk")
        print( myxtab.regtrk )
        
        if (length(keep.tm1) > 0){
            
            # Where there is regular tracking, we need to recode t to the overlapping values from tm1
            if (inherits(myxtab.regtrk, "integer")){
                # Occurs if myxtab only has 1 record
                # Need to output two variables from here: this.fromID and this.toID
                this.fromID <- keep.t[which(myxtab.regtrk > 0)]
                this.toID <- keep.tm1
                if (length(this.fromID) != length(this.toID)){
                    return(list(tnew, t, results, uniqID))
                }
            } 
        
            if (inherits(myxtab.regtrk, "data.frame")){
    #             browser()
                myxtab.regtrk <- myxtab.regtrk[which(rowSums(myxtab.regtrk) != 0),]  
                # Set up recoding data.frame
                myrecodes <- apply(myxtab.regtrk, 2, FUN=function(x){
                    cn <- colnames(myxtab.regtrk)
                    ii <- which(x>0)
                    return( as.numeric(attributes(ii)$names) )
                    } )
...
#             tnew <- cover(tnew, t.rc)
            ...
            # If there are no regular tracking overlaps ...
            return(list(tnew, t, results, uniqID))
        }
    } else {
        # If there's an error in keep.t
        browser()
        return(list(tnew, t, results, uniqID))
    }
}

generation3 <- function(tm1, t, tnew, myxtab, results, uniqID, mydate){
...
dissipation3 <- function(tm1, t, tnew, myxtab, results, uniqID, mydate, mydate.tm1){
...
merging3 <- function(tm1, t, tnew, myxtab, results, uniqID, mydate, mydate.tm1){
...
splitting3 <- function(tm1, t, tnew, myxtab, results, uniqID, mydate){
...
remaining3 <- function(tm1, t, tnew, myxtab, results, uniqID, mydate){

...
#         print(mydates[x])
        ...
        if (haveData){
            ...
            cl.se  <- zonal(mcsprecip.intense, mycl, fun=function(x, na.rm=T){x <- x[!is.na(x)]; return(sd(x)/sqrt(length(x)))})
            ...
            if (!exists("mydf")){
                mydf <- data.frame(zon.stats[,1:2], sdmin=zon.stats[,2]-zon.stats[,3], sdmax=zon.stats[,2]+zon.stats[,3], se=zon.stats[,"cl.se"], zon.stats[,c("count","landcover")], time=mydates[x])                
            } else {
                mydf <- rbind(mydf, data.frame(zon.stats[,1:2], sdmin=zon.stats[,2]-zon.stats[,3], sdmax=zon.stats[,2]+zon.stats[,3], se=zon.stats[,"cl.se"], zon.stats[,c("count","landcover")], time=mydates[x]))
            }
...
for (xx in ids){
    try(
        treepr <- aggregate(Precip ~ time, data=smalldata[smalldata$ID == xx & smalldata$Hour > 14 & smalldata$Hour < 21 & smalldata$Class==1,], FUN=mean)
        , silent=T)
    try(
        grasspr <- aggregate(Precip ~ time, data=smalldata[smalldata$ID == xx & smalldata$Hour > 14 & smalldata$Hour < 21 & smalldata$Class==2,], FUN=mean)
        , silent=T)
    try(
        bndrypr <- aggregate(Precip ~ time, data=smalldata[smalldata$ID == xx & smalldata$Hour > 14 & smalldata$Hour < 21 & smalldata$Class==4,], FUN=mean)
        , silent=T)
...
round(table(compall$PrecipTree > compall$PrecipGrass) / sum(table(compall$PrecipTree > compall$PrecipGrass)), 2)
round(table(compall$PrecipTree > compall$PrecipBndry) / sum(table(compall$PrecipTree > compall$PrecipBndry)), 2)
round(table(compall$PrecipGrass > compall$PrecipBndry) / sum(table(compall$PrecipGrass > compall$PrecipBndry)), 2)
